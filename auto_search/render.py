...
def render_outputs(brief_path: Path, docs_dir: Path = DEFAULT_DOCS_DIR) -> Dict[str, Path]:
    ...
    report_dir = docs_dir / "reports" / date_text
    ...
    report_url = report_public_url(date_text)
...
def render_section(section: Dict[str, Any]) -> str:
    ...
    trend = paragraphs(section.get("trend_summary", ""))
    bullets = section.get("trend_bullets") or []
    if bullets:
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <meta name="robots" content="noindex,nofollow">
  <meta http-equiv="refresh" content="0; url=reports/{date_text}/">
  <title>{title}</title>
</head>
<body>
  <p><a href="reports/{date_text}/">打开 {date_text} 简报</a></p>
</body>
</html>
"""
...
def build_feishu_card(brief: Dict[str, Any], report_url: str) -> Dict[str, Any]:
    ...
    page_summary = truncate_md(str(brief.get("page_summary") or ""), 900)
    if page_summary:
...
def section_markdown(section: Dict[str, Any]) -> str:
    ...
    trend = str(section.get("trend_summary") or "").strip()
    if trend:
