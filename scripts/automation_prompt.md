...
目标：生成一份中文 AI / 推荐系统研究情报日报，并推送到飞书。日报要像报纸科技新闻简报，不要硬写天级别“大趋势”。使用本 app 内你自身的 `gpt-5.5` 能力完成判断和写作；不要让项目代码调用外部 LLM API。
...
3. 生成 `runs/YYYY-MM-DD/brief.json`，结构必须符合 `candidates.json` 里的 `instructions_for_codex.schema`：
   - 语言：中文。
   - `page_summary` 必须像新闻导语和 nut graf：先说今天最重要的新情况，再交代背景、影响和需要继续观察的点。
   - 四个板块都要有：`recsys_research`、`llm_hotspots`、`data_centric_ai`、`ai_social_tools`。
   - 每个板块先写 `section_summary`，用记者写法说明“今天发生了什么、证据是什么、为什么值得看”，不要泛泛写“趋势”。
   - 每个板块可写 `news_bullets`，必须是事实、影响或明确观察点，不要写空泛判断。
